...
describe("operator routes",()=>{
  ...
  it("requires explicit audio routing",()=>{const adapter=new DeterministicStudioAdapter();location.hash="#/audio";render(<App adapter={adapter}/>);const routes=screen.getAllByRole("checkbox",{name:/Route to Program/});expect(routes.filter(r=>(r as HTMLInputElement).checked)).toHaveLength(1);fireEvent.click(routes[1]);expect(adapter.getSnapshot().audio[1].routed).toBe(true)})
