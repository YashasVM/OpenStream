#!/usr/bin/env bash
# Installs the OpenStream OBS plugin module on Linux.
# Mirrors tools/installer/Install-OpenStreamPlugin.ps1 semantics:
# replaces known plugin copies (canonical + legacy beta names) instead of
# leaving stale modules behind.
#
#   ./install-openstream-plugin-linux.sh [--so PATH] [--system] [--dest-dir DIR]
#
# Default (no flags): per-user install, no root required:
#   ${XDG_CONFIG_HOME:-$HOME/.config}/obs-studio/plugins/openstream-obs/bin/64bit/openstream-obs.so
# --system: system-wide install into the detected OBS plugin dir
#   (e.g. /usr/lib/obs-plugins); requires root.
# --dest-dir DIR: install openstream-obs.so into DIR instead of the user or
...
# Packaged no-argument resolution: a sibling openstream-obs.so next to this
# script (extracted tarball layout) wins over the repository build fallback.
if [[ -z "${PLUGIN_SO}" ]]; then
  if [[ -f "${SCRIPT_DIR}/openstream-obs.so" ]]; then
    PLUGIN_SO="${SCRIPT_DIR}/openstream-obs.so"
  else
    PLUGIN_SO="${SCRIPT_DIR}/../../obs-plugin/build/openstream-obs.so"
  fi
fi
if [[ ! -f "${PLUGIN_SO}" ]]; then
...
atomic_install() {
  local src="$1"
  local dir="$2"
  local target="${dir}/openstream-obs.so"
  local tmp=""
  tmp="$(mktemp "${dir}/.openstream-obs.so.XXXXXX")"
...
if [[ -n "${DEST_DIR}" ]]; then
  ...
  dest_dir="${config_home}/obs-studio/plugins/openstream-obs/bin/64bit"
  mkdir -p "${dest_dir}"
  atomic_install "${PLUGIN_SO}" "${dest_dir}"
  remove_stale_names "${dest_dir}"
