#!/usr/bin/env python3
"""Measure how quickly FFmpeg's SRT receiver exits after a bidirectional blackhole.
...
media to flow, then blackholes both directions and measures receiver exit latency.
"""
...
async def run_probe(timeout_us: int, warmup_s: float, max_wait_s: float) -> float:
    ffmpeg = ffmpeg_path()
    ...
    try:
        ...
        while state.forwarded_packets < 100 and time.monotonic() < deadline:
        ...
        if state.forwarded_packets < 100:
        ...
        return time.monotonic() - blackhole_at
    finally:
        terminate(sender)
        terminate(receiver)
...
def main() -> int:
    args = parse_args()
    if args.timeout_us <= 0 or args.warmup_s < 0 or args.max_wait_s <= 0:
        print("timeout/wait values must be positive", file=sys.stderr)
        return 2
    try:
        elapsed = asyncio.run(run_probe(args.timeout_us, args.warmup_s, args.max_wait_s))
